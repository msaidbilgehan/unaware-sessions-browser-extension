...
- **新建会话入口：** 将底部新建按钮移至 popup 顶部工具栏，与主题切换按钮并排展示

### Fixed

- **WebDAV 备份轮换：** 设置最大备份数后立即清理多余旧备份，修复分页删除后页码越界导致无数据的 bug
- **弹出层站点统计刷新：** 会话切换、删除完成后重新读取权威站点数据，避免站点数量和会话角标残留旧值
- **删除会话后的标签页残留：** 删除会话时同步解除所有标签页绑定、清理隔离规则并更新角标，避免弹出层重开后读到已删除会话
- **严格全量保存站点数据：** 手动保存会先删除当前会话与站点的旧 Cookie/Storage 快照，再写入完整新快照；抓取失败时不再保留旧数据
- **新建会话自动保存：** 新会话绑定当前标签页后自动捕获一次完整站点数据，无需再次点击保存/检测按钮
- **站点会话数量虚高：** 统计站点会话时过滤已删除会话遗留的孤儿快照，确保 popup 角标与实际会话、设置页数量一致
- **孤儿快照污染：** 启动、导入/恢复及导出前永久清理无有效会话归属的 Cookie/Storage 快照；自动检测同时忽略无效会话，防止错误绑定和同步传播
