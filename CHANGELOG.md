...
- **IDB `deleteDatabase.onblocked` now rejects:** When another connection blocks the delete, the restore is skipped entirely instead of silently opening the existing database with a potentially mismatched schema
...
- **IDB restore corrupts database on blocked delete:** `deleteDatabase.onblocked` resolved the promise as if deletion succeeded, causing the subsequent `open()` to reuse the existing database without triggering `onupgradeneeded` — schema mismatches and silent data corruption. Now rejects and skips the restore for that database
