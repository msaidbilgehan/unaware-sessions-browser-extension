...
- **IDB restore corrupts database on blocked delete:** `deleteDatabase.onblocked` previously resolved the promise as if deletion succeeded, causing the subsequent `open()` to reuse the existing database without triggering `onupgradeneeded` — schema mismatches and silent data corruption. Now logs a warning and proceeds with the restore attempt
